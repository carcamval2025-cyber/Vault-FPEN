...
(function () {
  ...
  if (menuBtn && menu) {
    ...
    var nSemana = 0;
    document.querySelectorAll(".isla-semanas > *").forEach(function (a) {
      var actual = a.getAttribute("aria-current") === "page" ? ' aria-current="page"' : "";
      if (a.hasAttribute("data-control")) {
        html += "<li>" + entra('<a href="' + a.getAttribute("href") + '"' + actual + '><span class="n">C1</span><span class="tit">' +
          a.getAttribute("data-control") + "</span></a>") + "</li>";
        return;
      }
      var num = (nSemana < 9 ? "0" : "") + (nSemana + 1), tit = TITULOS[nSemana++];
      if (a.tagName !== "A") html += "<li>" + entra('<span class="futura-m"><span class="n">' + num + '</span><span class="tit">' + tit + "</span></span>") + "</li>";
      else html += "<li>" + entra('<a href="' + a.getAttribute("href") + '"' + actual + '><span class="n">' + num + '</span><span class="tit">' + tit + "</span></a>") + "</li>";
    });
    html += "</ol></nav>";
  ...
  if ("IntersectionObserver" in window && !sinMovimiento) {
    var objetivos = document.querySelectorAll("main h2, figure.lamina, .ciclo, .ejercicio, .momentos, .permisos, .quiz-score, .nav-semanas, .temario > li, .pesos > div, .recorrido > .card");
    ...
    objetivos.forEach(function (el) {
      el.classList.add("rv");
      if (el.parentElement && (el.parentElement.classList.contains("temario") || el.parentElement.classList.contains("recorrido"))) {
